...
- 實際頁數：封面 1 張＋內容 15 張，共 16 張 `.sheet`。
- 每張 A4 已拆成獨立 TSX 頁面元件；入口頁只負責載入講義閱讀器。
- 單頁密度規則已移至各頁 CSS Module；共用視覺與列印規則仍由主 CSS 管理。
- 自動測試已固定檢查16張頁面、`cover → p01 → … → p15` 順序及 Step 4 內容順序。
...
| `p06` | 6 | `Page06.tsx` | STEP 2｜把學習證據變成可填寫的工作頁 | 固定包含通用提示語、修訂清單、學習單表格與全部操作產出 |
| `p07` | 7 | `Page07.tsx` | STEP 3｜依講義規劃建立 HTML 簡報 | 講義／簡報對照、八頁投影片地圖與案例提示語 |
| `p08` | 8 | `Page08.tsx` | STEP 3｜規劃投影片地圖 | 六頁範例地圖、通用提示語、修改原則與生成流程前半 |
| `p09` | 9 | `Page09.tsx` | STEP 3｜完成存檔、預覽與修訂 | 接續步驟 3、提示詞、修訂詞與錯誤修正對照 |
| `p10` | 10 | `Page10.tsx` | STEP 4｜將互動內容做成網頁並傳送到 Netlify | 情境、互動示意、案例提示語、五題規格、通用模板順序固定 |
| `p11` | 11 | `Page11.tsx` | STEP 4｜完成互動規格並登入 Netlify | 書寫框、註冊畫面、四個登入步驟與用途提示 |
| `p12` | 12 | `Page12.tsx` | STEP 4｜第一次拖放部署 | Drop 畫面、部署步驟 1–5 與線上驗收 |
| `p13` | 13 | `Page13.tsx` | STEP 4｜網站有改動時，整包重新部署 | 更新步驟、問題速查、公開前檢查與官方來源 |
| `p14` | 14 | `Page14.tsx` | STEP 5｜其他延伸部分 | Agent 任務表、四項判斷與下一個真實工作書寫框 |
| `p15` | 15 | `Page15.tsx` | FINISH｜成果交付與最後驗收 | 六項成果、三個書寫框、收束與連結 |
...
- 重構前後皆為16張 `.sheet`。
- 重構前後可見文字完全一致，共 7,509 個正規化字元。
- 尚未執行瀏覽器視覺檢查或列印預覽，因此不得宣稱 A4 視覺分頁已完成實際預覽驗證。
